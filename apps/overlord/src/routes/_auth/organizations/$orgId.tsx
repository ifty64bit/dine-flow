...
function OrgDetailPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn:  async () => {
      const res = await client.api.overlord.organizations[':id'].$get({ param: { id: orgId } })
  ...
  const toggleActive = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['overlord', 'org', orgId] }),
  })

  const changeSubStatus = useMutation({
    mutationFn: async (status: 'active' | 'trialing' | 'past_due' | 'cancelled' | 'paused') => {
      const res = await client.api.overlord.organizations[':id'].subscription.$patch({
        param: { id: orgId },
        json:  { status },
      })
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <section className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-semibold text-zinc-300">Subscription</h2>
            </div>
              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-sm font-semibold text-zinc-200">
                    {org.subscription.planName} · ৳{org.subscription.monthlyPrice}/mo
                  </span>
                </div>
                <InfoRow label="Period start" value={new Date(org.subscription.currentPeriodStart).toLocaleDateString()} />
                {org.subscription.currentPeriodEnd && (
                  <InfoRow label="Period end" value={new Date(org.subscription.currentPeriodEnd).toLocaleDateString()} />
                )}
                {org.subscription.cancelledAt && (
                  <InfoRow label="Cancelled" value={new Date(org.subscription.cancelledAt).toLocaleDateString()} />
                )}

                {/* Quick status actions */}
                <div className="flex gap-2 mt-4 flex-wrap">
                  {(['active', 'trialing', 'past_due', 'cancelled', 'paused'] as const).map((s) => (
                    <button
                      key={s}
                      onClick={() => changeSubStatus.mutate(s)}
                      disabled={changeSubStatus.isPending || org.subscription?.status === s}
          ...
          <section className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-zinc-800">
              ...
              <h2 className="text-sm font-semibold text-zinc-300">
                Branches <span className="text-zinc-600 font-normal">({org.branches.length})</span>
              </h2>
            </div>
                ...
                {org.branches.map((b) => (
                  <div key={b.id} className="flex items-center justify-between px-5 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="text-sm font-medium text-zinc-300">{b.name}</p>
                        {b.address && <p className="text-xs text-zinc-600">{b.address}</p>}
                      </div>
                    </div>
                    <div className="flex items-center gap-3 text-xs text-zinc-500">
                      <span>{b.tableCount} tables</span>
                      <span>{b.memberCount} staff</span>
                      <span className={`w-1.5 h-1.5 rounded-full ${b.isActive ? 'bg-emerald-500' : 'bg-zinc-600'}`} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <section className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-zinc-800">
              ...
              <h2 className="text-sm font-semibold text-zinc-300">
                Members <span className="text-zinc-600 font-normal">({org.members.length})</span>
              </h2>
            </div>
                ...
                {org.members.map((m) => (
                  <div key={m.id} className="flex items-center justify-between px-5 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="text-sm font-medium text-zinc-300">{m.name}</p>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {m.branchName && (
                        <span className="text-xs text-zinc-600 hidden sm:block">{m.branchName}</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div className="space-y-5">
          <section className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
            ...
            <InfoRow label="Service charge" value={`${org.serviceChargeRate}%`} />
            <InfoRow label="Tax inclusive"  value={org.taxInclusive ? 'Yes' : 'No'} />
            <InfoRow label="Status"         value={
              <span className={`font-medium ${org.isActive ? 'text-emerald-400' : 'text-zinc-500'}`}>
                {org.isActive ? 'Active' : 'Inactive'}
              </span>
            } />
            <InfoRow label="Created" value={new Date(org.createdAt).toLocaleDateString()} />
            {org.trialEndsAt && (
              <InfoRow label="Trial ends" value={new Date(org.trialEndsAt).toLocaleDateString()} />
            )}
          </section>
        </div>
      </div>
