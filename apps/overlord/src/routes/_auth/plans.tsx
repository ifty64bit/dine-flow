...
import { AlertCircle, Check, CreditCard, Pencil, Plus, X } from "lucide-react";
...
const PlanSchema = v.object({
	...
	slug: v.pipe(
		v.string(),
		v.regex(/^[a-z0-9-]+$/, "Lowercase letters, numbers, hyphens only"),
	),
	monthlyPrice: v.pipe(
		v.string(),
		v.regex(/^\d+(\.\d{1,2})?$/, "Invalid price"),
	),
...
function Field({
	...
	set: (key: keyof PlanForm, val: string) => void;
}) {
	return (
			...
			<div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
				...
				<form onSubmit={handleSubmit} className="p-5 space-y-4">
					...
					<div className="grid grid-cols-3 gap-4">
						...
						/>
						<Field
							label="Max Tables/Branch"
							name="maxTablesPerBranch"
							type="number"
							form={form}
							errors={errors}
							set={set}
...
function PlansPage() {
	...
	const [editing, setEditing] = useState<{ id: number; form: PlanForm } | null>(
		null,
	);
	...
	const createPlan = useMutation({
		mutationFn: async (form: PlanForm) => {
			const res = await client.api.overlord.plans.$post({
		...
		onSuccess: () => {
			qc.invalidateQueries({ queryKey: ["overlord", "plans"] });
			setCreating(false);
		},
	});
				...
				{plans.map((plan) => (
					...
					>
						...
						<div className="space-y-1.5 text-xs text-zinc-500">
							<p>
								Branches:{" "}
								<span className="text-zinc-300">{plan.maxBranches}</span>
							</p>
							<p>
								Users/branch:{" "}
								<span className="text-zinc-300">{plan.maxUsersPerBranch}</span>
							</p>
							<p>
								Tables/branch:{" "}
								<span className="text-zinc-300">{plan.maxTablesPerBranch}</span>
							</p>
							<p>
								Subscribers:{" "}
								<span className="text-zinc-300">{plan.subscriberCount}</span>
							</p>
						</div>
						...
						<div className="flex gap-2 pt-1">
							...
							<button
								type="button"
								onClick={() =>
									togglePlan.mutate({ id: plan.id, isActive: !plan.isActive })
								}
