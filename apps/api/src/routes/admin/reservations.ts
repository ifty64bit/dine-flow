...
adminReservationRoutes.use('*', requireAuth, roleGuard('manager'))

adminReservationRoutes.get('/', async (c) => {
  const branchId = c.req.query('branchId')
  const date = c.req.query('date')

  const conditions: ReturnType<typeof eq>[] = []
  if (branchId) conditions.push(eq(reservations.branchId, branchId))
...
adminReservationRoutes.get('/:id', async (c) => {
...
adminReservationRoutes.put(
  '/:id',
  zValidator('json', createReservationSchema.partial()),
  async (c) => {
    const id = c.req.param('id')
    const body = c.req.valid('json')
...
adminReservationRoutes.delete('/:id', async (c) => {
  const id = c.req.param('id')
