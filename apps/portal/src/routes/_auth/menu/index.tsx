...
const STATIONS = ['grill', 'fryer', 'salad', 'drinks', 'dessert', 'general'] as const
...
function CategoryForm({
  ...
  onSubmit: (data: { name: string; description: string; sortOrder: number; isActive: boolean }) => void
  loading: boolean
}) {
  ...
  return (
    <form
      onSubmit={(e) => { e.preventDefault(); onSubmit({ name, description, sortOrder, isActive }) }}
      className="space-y-4"
    >
      <div>
        <label className={labelClass()}>Name *</label>
        <input className={inputClass()} value={name} onChange={e => setName(e.target.value)} placeholder="Starters" required />
      </div>
      <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select className={inputClass()} value={isActive ? '1' : '0'} onChange={e => setIsActive(e.target.value === '1')}>
...
function ItemForm({
  ...
  const [station, setStation] = useState<StationOption>((initial?.station as StationOption) ?? 'general')
  const [prepTimeMin, setPrepTimeMin] = useState(initial?.prepTimeMin ?? 15)
  const [isAvailable, setIsAvailable] = useState(initial?.isAvailable ?? true)

  return (
    <form
      onSubmit={(e) => { e.preventDefault(); onSubmit({ name, description, basePrice, categoryId, station, prepTimeMin, isAvailable }) }}
      className="space-y-4"
    >
      <div>
        <label className={labelClass()}>Name *</label>
        <input className={inputClass()} value={name} onChange={e => setName(e.target.value)} placeholder="Grilled Chicken" required />
      </div>
      <div>
        <label className={labelClass()}>Description</label>
        <input className={inputClass()} value={description} onChange={e => setDescription(e.target.value)} placeholder="Optional description" />
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input type="number" className={inputClass()} value={prepTimeMin} onChange={e => setPrepTimeMin(Number(e.target.value))} min={0} />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
        ...
        <div>
          ...
          <select className={inputClass()} value={station} onChange={e => setStation(e.target.value as StationOption)}>
            {STATIONS.map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>)}
          </select>
        </div>
      </div>
      <div>
        <label className={labelClass()}>Availability</label>
        <select className={inputClass()} value={isAvailable ? '1' : '0'} onChange={e => setIsAvailable(e.target.value === '1')}>
...
function MenuPage() {
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ['portal', 'menu'] })
  ...
  const addCat = useMutation({
    mutationFn: async (body: Parameters<typeof client.api.v1.admin.menu.categories.$post>[0]['json']) => {
      const res = await client.api.v1.admin.menu.categories.$post({ json: body })
  ...
  const editCat = useMutation({
    mutationFn: async ({ id, body }: { id: number; body: Parameters<typeof client.api.v1.admin.menu.categories.$post>[0]['json'] }) => {
      const res = await client.api.v1.admin.menu.categories[':id'].$put({ param: { id: String(id) }, json: body })
  ...
  const deleteCat = useMutation({
    mutationFn: async (id: number) => {
      const res = await client.api.v1.admin.menu.categories[':id'].$delete({ param: { id: String(id) } })
  ...
  const addItem = useMutation({
    mutationFn: async (body: Parameters<typeof client.api.v1.admin.menu.items.$post>[0]['json']) => {
  ...
  const editItem = useMutation({
    mutationFn: async ({ id, body }: { id: number; body: Parameters<typeof client.api.v1.admin.menu.items.$post>[0]['json'] }) => {
      const res = await client.api.v1.admin.menu.items[':id'].$put({ param: { id: String(id) }, json: body })
  ...
  const deleteItem = useMutation({
    mutationFn: async (id: number) => {
      const res = await client.api.v1.admin.menu.items[':id'].$delete({ param: { id: String(id) } })
  ...
  const isLoading = catQuery.isLoading || (tab === 'items' && itemQuery.isLoading)
  ...
  return (
    <div className="p-6 space-y-5 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => setModal(tab === 'categories' ? { type: 'add-category' } : { type: 'add-item' })}
          ...
          {categories.map(cat => (
            <div key={cat.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
              ...
              <div className="flex items-center gap-3 px-4 py-3">
                <button onClick={() => toggleExpand(cat.id)} className="text-zinc-500 hover:text-zinc-300 transition-colors">
                  {expanded.has(cat.id)
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {!cat.isActive && (
                      <span className="text-xs px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-500">inactive</span>
                    )}
                  </div>
                  {cat.description && (
                    <p className="text-xs text-zinc-600 truncate">{cat.description}</p>
                  )}
                </div>
                <span className="text-xs text-zinc-600 shrink-0">{cat.items.length} items</span>
                ...
                <div className="border-t border-zinc-800 divide-y divide-zinc-800/60">
                  {cat.items.map((item: Item) => (
                    <div key={item.id} className="flex items-center gap-3 px-4 py-2.5 pl-11">
                      ...
                      <span className="flex-1 text-xs text-zinc-300 truncate">{item.name}</span>
                      {item.station && (
                        <span className="text-xs text-zinc-600 shrink-0">{item.station}</span>
                      )}
                      <span className="text-xs text-zinc-400 shrink-0">৳{item.basePrice}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {items.map((item: Item) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm font-medium text-zinc-300 shrink-0">৳{item.basePrice}</span>
                <button
      ...
      {modal?.type === 'add-category' && (
        <Modal title="Add Category" onClose={() => setModal(null)}>
          ...
          />
          {addCat.isError && (
            <p className="text-xs text-red-400 mt-2">{(addCat.error as Error).message}</p>
          )}
        </Modal>
      )}
      ...
      {modal?.type === 'edit-category' && (
        <Modal title="Edit Category" onClose={() => setModal(null)}>
          <CategoryForm
            ...
            onSubmit={data => editCat.mutate({ id: modal.cat.id, body: data })}
          />
          {editCat.isError && (
            <p className="text-xs text-red-400 mt-2">{(editCat.error as Error).message}</p>
          )}
        </Modal>
      )}
      ...
      {modal?.type === 'add-item' && (
        <Modal title="Add Item" onClose={() => setModal(null)}>
          ...
          />
          {addItem.isError && (
            <p className="text-xs text-red-400 mt-2">{(addItem.error as Error).message}</p>
          )}
        </Modal>
      )}
      ...
      {modal?.type === 'edit-item' && (
        <Modal title="Edit Item" onClose={() => setModal(null)}>
          <ItemForm
            ...
            onSubmit={data => editItem.mutate({ id: modal.item.id, body: data })}
          />
          {editItem.isError && (
            <p className="text-xs text-red-400 mt-2">{(editItem.error as Error).message}</p>
          )}
        </Modal>
      )}
    </div>
