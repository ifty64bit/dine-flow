	...
	return (
		...
		>
			<div className="grid grid-cols-2 gap-3">
				<div>
					<label htmlFor="table-number" className={labelClass()}>
						Table Number *
					</label>
					<input
						type="number"
			...
			<div className="grid grid-cols-2 gap-3">
				...
				<div>
					<label htmlFor="table-class" className={labelClass()}>
						Table Class *
					</label>
					<select
						className={inputClass()}
						value={tableClassId}
						onChange={(e) => setTableClassId(Number(e.target.value))}
						required
					>
						{classes.map((c) => (
			...
			<div className="grid grid-cols-2 gap-3">
				<div>
					...
					<select
						...
						value={shape}
						onChange={(e) =>
							setShape(e.target.value as "round" | "square" | "rectangle")
						}
					>
...
function TableClassForm({
	...
	const [priceMultiplier, setPriceMultiplier] = useState(
		initial?.priceMultiplier ?? "1.00",
	);
	const [badgeColor, setBadgeColor] = useState(
		initial?.badgeColor ?? "#6B7280",
	);
	...
	return (
		...
		>
			...
			<div className="grid grid-cols-2 gap-3">
				<div>
					<label htmlFor="price-multiplier" className={labelClass()}>
						Price Multiplier
					</label>
					<input
						className={inputClass()}
				...
				<div>
					<label htmlFor="badge-color" className={labelClass()}>
						Badge Color
					</label>
...
function TablesPage() {
	...
	const invalidate = () =>
		qc.invalidateQueries({ queryKey: ["portal", "tables"] });
	...
	const addTable = useMutation({
		mutationFn: async (
			body: Parameters<typeof client.api.v1.admin.tables.$post>[0]["json"],
		) => {
	...
	const editTable = useMutation({
		mutationFn: async ({
			...
			const res = await client.api.v1.admin.tables[":id"].$put({
				param: { id: String(id) },
				json: body,
			});
	...
	const deleteTable = useMutation({
		mutationFn: async (id: number) => {
			const res = await client.api.v1.admin.tables[":id"].$delete({
				param: { id: String(id) },
			});
	...
	const generateQr = useMutation({
		mutationFn: async (tableId: number) => {
			...
			return res.json() as Promise<{
				data: { qrImageUrl: string; qrUrl: string };
			}>;
		},
		onSuccess: () => invalidate(),
	});
	...
	const addClass = useMutation({
		mutationFn: async (
			body: Parameters<
				typeof client.api.v1.admin.tables.classes.$post
			>[0]["json"],
		) => {
			const res = await client.api.v1.admin.tables.classes.$post({
				json: body,
			});
	...
	const editClass = useMutation({
		mutationFn: async ({
			...
			const res = await client.api.v1.admin.tables.classes[":id"].$put({
				param: { id: String(id) },
				json: body,
			});
	...
	const deleteClass = useMutation({
		mutationFn: async (id: number) => {
			const res = await client.api.v1.admin.tables.classes[":id"].$delete({
				param: { id: String(id) },
			});
	...
	const isLoading =
		...
		(tab === "classes" && classQuery.isLoading) ||
		branchQuery.isLoading;
	const isError =
		tableQuery.isError ||
		(tab === "classes" && classQuery.isError) ||
		branchQuery.isError;
		...
		<div className="p-6 space-y-5 max-w-5xl mx-auto">
			...
			<div className="flex items-center justify-between">
				...
				<button
					...
					onClick={() =>
						setModal(
							tab === "tables" ? { type: "add-table" } : { type: "add-class" },
						)
					}
						...
						return (
							...
							>
								...
								<div>
									...
									{table.floorName && (
										<p className="text-xs text-zinc-600">{table.floorName}</p>
									)}
								</div>
								...
								>
									<option value="vacant" className="bg-zinc-800 text-zinc-100">
										vacant
									</option>
									<option
										value="occupied"
									...
									<option value="merged" className="bg-zinc-800 text-zinc-100">
										merged
									</option>
								</select>
							</div>
				...
				<div className="space-y-2">
					{classes.length === 0 && (
						<div className="flex flex-col items-center justify-center py-16 gap-2 text-zinc-600">
							...
							<p className="text-sm">
								No table classes yet — add one to get started
							</p>
						</div>
					)}
					{classes.map((cls) => (
						...
						>
							...
							<div className="flex-1 min-w-0">
								...
								<p className="text-xs text-zinc-500">
									Multiplier {cls.priceMultiplier} ·{" "}
									{tables.filter((t) => t.tableClassId === cls.id).length}{" "}
									tables
								</p>
							</div>
				...
				<Modal title="Add Table" onClose={() => setModal(null)}>
					...
					{addTable.isError && (
						<p className="text-xs text-red-400 mt-2">
							{(addTable.error as Error).message}
						</p>
					)}
				</Modal>
			)}
			...
			{modal?.type === "edit-table" && (
				<Modal title="Edit Table" onClose={() => setModal(null)}>
					<TableForm
						...
						onSubmit={(data) =>
							editTable.mutate({ id: modal.table.id, body: data })
						}
					/>
					{editTable.isError && (
						<p className="text-xs text-red-400 mt-2">
							{(editTable.error as Error).message}
						</p>
					)}
				</Modal>
			)}
			...
			{modal?.type === "add-class" && (
				<Modal title="Add Table Class" onClose={() => setModal(null)}>
					...
					{addClass.isError && (
						<p className="text-xs text-red-400 mt-2">
							{(addClass.error as Error).message}
						</p>
					)}
				</Modal>
			)}
			...
			{modal?.type === "edit-class" && (
				<Modal title="Edit Table Class" onClose={() => setModal(null)}>
					<TableClassForm
						...
						onSubmit={(data) =>
							editClass.mutate({ id: modal.cls.id, body: data })
						}
					/>
					{editClass.isError && (
						<p className="text-xs text-red-400 mt-2">
							{(editClass.error as Error).message}
						</p>
					)}
				</Modal>
			)}
			...
			{modal?.type === "qr" && (
				<Modal
					title={`Table ${modal.table.number} QR`}
					...
					<div className="space-y-4">
						...
						{generateQr.isError && (
							<p className="text-xs text-red-400 text-center">
								{(generateQr.error as Error).message}
							</p>
						)}
					</div>
				</Modal>
