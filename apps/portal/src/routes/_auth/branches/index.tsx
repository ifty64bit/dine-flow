...
function BranchForm({
  ...
  onSubmit: (data: { name: string; address: string; phone: string; isActive: boolean }) => void
  loading: boolean
}) {
  ...
  return (
    ...
    >
      <div>
        ...
        <input className={inputClass()} value={name} onChange={(e) => setName(e.target.value)} placeholder="Main Branch" required />
      </div>
      <div>
        <label className={labelClass()}>Address</label>
        <input className={inputClass()} value={address} onChange={(e) => setAddress(e.target.value)} placeholder="123 Restaurant St" />
      </div>
      <div>
...
function BranchesPage() {
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ['portal', 'branches'] })
  ...
  const addBranch = useMutation({
    mutationFn: async (body: Parameters<typeof client.api.v1.admin.branches.$post>[0]['json']) => {
  ...
  const editBranch = useMutation({
    mutationFn: async ({ id, body }: { id: number; body: Parameters<typeof client.api.v1.admin.branches.$post>[0]['json'] }) => {
      const res = await client.api.v1.admin.branches[':id'].$put({ param: { id: String(id) }, json: body })
  ...
  const deleteBranch = useMutation({
    mutationFn: async (id: number) => {
      const res = await client.api.v1.admin.branches[':id'].$delete({ param: { id: String(id) } })
    ...
    <div className="p-6 space-y-5 max-w-5xl mx-auto">
          ...
          {branches.map((branch) => (
            ...
            >
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                <button
                  onClick={() => { if (confirm(`Delete "${branch.name}"?`)) deleteBranch.mutate(branch.id) }}
      ...
      {modal?.type === 'add' && (
        <Modal title="Add Branch" onClose={() => setModal(null)}>
          ...
          />
          {addBranch.isError && (
            <p className="text-xs text-red-400 mt-2">{(addBranch.error as Error).message}</p>
          )}
        </Modal>
      )}
      ...
      {modal?.type === 'edit' && (
        <Modal title="Edit Branch" onClose={() => setModal(null)}>
          <BranchForm
            ...
            onSubmit={(data) => editBranch.mutate({ id: modal.branch.id, body: data })}
          />
          {editBranch.isError && (
            <p className="text-xs text-red-400 mt-2">{(editBranch.error as Error).message}</p>
          )}
        </Modal>
      )}
    </div>
