...
async function seed() {
  ...
  // Table classes
  const [regularClass] = await db.insert(tableClasses).values({
    name: 'Regular',
    slug: 'regular',
  ...
  const [vipClass] = await db.insert(tableClasses).values({
    name: 'VIP',
    slug: 'vip',
  ...
  // Tables
  const tableData = [
    { number: 1, capacity: 4, tableClassId: regularClass.id, positionX: '50', positionY: '50' },
    { number: 2, capacity: 4, tableClassId: regularClass.id, positionX: '200', positionY: '50' },
    { number: 3, capacity: 6, tableClassId: regularClass.id, positionX: '50', positionY: '200' },
    { number: 4, capacity: 6, tableClassId: regularClass.id, positionX: '200', positionY: '200' },
    { number: 5, capacity: 2, tableClassId: vipClass.id, positionX: '400', positionY: '50' },
    { number: 6, capacity: 4, tableClassId: vipClass.id, positionX: '400', positionY: '200' },
  ]
  const insertedTables = await db.insert(tables).values(
    tableData.map((t) => ({
      ...t,
      floorId: floor.id,
  ...
  // Menu Categories
  const [starters] = await db.insert(menuCategories).values({
    name: 'Starters',
  ...
  const [mains] = await db.insert(menuCategories).values({
    name: 'Mains',
  ...
  const [desserts] = await db.insert(menuCategories).values({
    name: 'Desserts',
  ...
  await db.insert(modifiers).values([
    { groupId: sizeGroup.id, name: 'Small', priceAdjustment: '-20', sortOrder: 1 },
    { groupId: sizeGroup.id, name: 'Medium', priceAdjustment: '0', sortOrder: 2 },
    { groupId: sizeGroup.id, name: 'Large', priceAdjustment: '30', sortOrder: 3 },
  ])

  // Extras modifiers
  await db.insert(modifiers).values([
    { groupId: extrasGroup.id, name: 'Extra Cheese', priceAdjustment: '30', sortOrder: 1 },
    { groupId: extrasGroup.id, name: 'Extra Sauce', priceAdjustment: '20', sortOrder: 2 },
    { groupId: extrasGroup.id, name: 'Extra Spicy', priceAdjustment: '10', sortOrder: 3 },
  ])

  // Menu Items — Starters
  const starterItems = await db.insert(menuItems).values([
    {
      categoryId: starters.id,
      name: 'Vegetable Spring Rolls',
      description: 'Crispy spring rolls filled with seasonal vegetables and glass noodles',
      basePrice: '150',
      prepTimeMin: 10,
  ...
  // Menu Items — Mains
  const mainItems = await db.insert(menuItems).values([
    {
      categoryId: mains.id,
    ...
    },
    {
      categoryId: mains.id,
      name: 'Wagyu Burger',
      description: 'Premium A5 Wagyu beef patty with caramelized onions and truffle mayo',
      basePrice: '1200',
      prepTimeMin: 25,
      dietaryTags: [],
      isAvailable: true,
  ...
  // Menu Items — Desserts
  const dessertItems = await db.insert(menuItems).values([
    {
    ...
    },
    {
      categoryId: desserts.id,
      name: 'Rasgolla',
      description: 'Traditional Bengali soft cottage cheese balls in light sugar syrup',
      basePrice: '120',
      prepTimeMin: 5,
      dietaryTags: ['vegetarian'],
      isAvailable: true,
    ...
    },
    {
      categoryId: desserts.id,
      name: 'Mango Kulfi',
      description: 'Creamy frozen dessert made with reduced milk and fresh Alphonso mango',
      basePrice: '150',
      prepTimeMin: 3,
      dietaryTags: ['vegetarian'],
      isAvailable: true,
  ...
  // Menu Items — Drinks
  const drinkItems = await db.insert(menuItems).values([
    {
      categoryId: drinks.id,
      name: 'Fresh Lime Soda',
      description: 'Freshly squeezed lime with sparkling water and a hint of mint',
      basePrice: '80',
      prepTimeMin: 5,
    ...
    },
    {
      categoryId: drinks.id,
      name: 'Mango Lassi',
      description: 'Thick and creamy yogurt blended with sweet Alphonso mango',
      basePrice: '120',
      prepTimeMin: 5,
    ...
    },
    {
      categoryId: drinks.id,
      name: 'Masala Chai',
      description: 'Aromatic spiced tea brewed with cardamom, ginger, and cinnamon',
      basePrice: '60',
      prepTimeMin: 8,
      dietaryTags: ['vegetarian', 'gluten-free'],
      isAvailable: true,
  ...
  // Order counter
  await db.insert(orderCounters).values({ id: 1, currentValue: 0 }).onConflictDoNothing()
